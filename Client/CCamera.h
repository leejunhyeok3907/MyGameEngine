...
private:
	...
	Vec2 m_vDiff; //해상도 중앙 위치와, 카메라 LookAt간의 차이 값
...
public:
	...
	Vec2 GetClientPos(Vec2 _vRenderPos) { return _vRenderPos + m_vDiff; }
	...
	void FadeIn(float _fDuration)
	{
		...
		if (0.f == _fDuration) assert(nullptr);

		m_listCamEffect.push_back(ef);
	}
	void FadeOut(float _fDuration)//몇초에 걸쳐서 효과를 넣을지
	{
		tCamEffect ef = {};
...
public:
	//최초로 싱글톤이 생성되는 시점에 해상도 정보를 모를수도 있으므로 초기화 시점을 정확하게 정해줄수있는 함수가 필요함.
	void init();
	void update();
	void render(HDC _dc);
