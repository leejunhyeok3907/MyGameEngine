...
int CSound::Load(const wchar_t* _strPath)
{
...
{
	//Play 함수의 3번째 변수는 사운드를 반복재생 할 것인지 아닌지를 결정한다.
	if (_bLoop)
...
bool CSound::LoadWaveSound(const wstring& _strPath)
{
	...
	if (nullptr == hFile)
	{
		MessageBox(NULL, L"사운드 리로스 경로에 파일 없음", L"사운드 로딩 실패", MB_OK);
		return false;
	}
	...
	{
		MessageBox(NULL, L"사운드버퍼 생성 실패", L"Error!", MB_OK);
		return false;
	}
